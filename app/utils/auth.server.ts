...
export async function requireAuth(request: Request, context: any) {
  ...
  const accessToken = cookies.wider_access_token || cookies.wider_shared_access_token || access;
  const refreshToken = cookies.wider_access_token || cookies.wider_shared_refresh_token || refresh;
